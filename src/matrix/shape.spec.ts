...
import { MatrixShape, RowShape, ColumnShape, SquareShape } from './shape';
...
describe('The Matrix shape and derrived classes', () => {
  ...
  it('Given an invalid 3x1 RowShape, it should throw an error', () => {
    expect(() => new RowShape(3,1)).to.throw('Incorrect sizes for matrix shape');
  });

  it('Given an invalid 1x3 ColumnShape, it should throw an error', () => {
    expect(() => new ColumnShape(1,3)).to.throw('Incorrect sizes for matrix shape');
  });

  it('Given an invalid 1x3 SquareShape, it should throw an error', () => {
    expect(() => new SquareShape(1,3)).to.throw('Incorrect sizes for matrix shape');
  });
});
